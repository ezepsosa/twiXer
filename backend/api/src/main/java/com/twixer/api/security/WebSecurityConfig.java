...
public class WebSecurityConfig { // extends WebSecurityConfigurerAdapter {
	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		http.csrf(csrf -> csrf.disable()) // Deshabilitar CSRF
				.exceptionHandling(exception -> exception.authenticationEntryPoint(unauthorizedHandler)) // Manejo de
																											// excepciones
				.sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS)) // Sin
																												// estado
				.authorizeHttpRequests(auth -> auth.requestMatchers("/api/v1/auth/**").permitAll() // Permitir acceso a
																									// auth
						.requestMatchers("/api/v1/test/**").permitAll() // Permitir acceso a test
